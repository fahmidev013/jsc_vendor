...
import 'package:jsc_barbershop/bloc/chat/image_send_sheet.dart';
...
import 'package:jsc_barbershop/screens/chat/fancy_button.dart';
import 'package:jsc_barbershop/screens/chat/video_upload_dialog.dart';
...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ChatBloc() : super(ChatInitial()) {
    on<FetchChatArgumentsEvent>((event, emit) async {
      ...
      add(UpdateChatEvent());
      ApiService()
          .fetchMyUserDetails(conversation.user?.userid ?? -1)
          .then((value) {
  ...
  Future<void> chatMessage({
    ...
    if (salonData?.isNotification == 1) {
      ApiService().pushNotification(
          ...
          title:
              salonData?.fullname ?? AppLocalizations.of(Get.context!)!.unknown,
