...
import 'package:jsc_barbershop/model/cat/categories.dart' as cat;
import 'package:jsc_barbershop/model/earninghistory/earning_history.dart';
import 'package:jsc_barbershop/model/faq/faqs.dart';
...
import 'package:jsc_barbershop/model/request/request_details.dart'
    as request_details;
import 'package:jsc_barbershop/model/rest/get_path.dart';
import 'package:jsc_barbershop/model/rest/rest_response.dart';
import 'package:jsc_barbershop/model/review/salon_review.dart';
import 'package:jsc_barbershop/model/service/services.dart' as services;
import 'package:jsc_barbershop/model/setting/setting.dart';
import 'package:jsc_barbershop/model/staff/staff.dart';
import 'package:jsc_barbershop/model/user/salon.dart';
import 'package:jsc_barbershop/model/user/salon_user.dart';
...
class ApiService {
  ...
  Future<Salon> updateSalonDetails({
    ...
    if (isServeOutSide != null) {
      request.fields[ConstRes.isServeOutSide] = isServeOutSide ? '1' : '0';
    }
    if (isPayAfterService != null) {
      request.fields[ConstRes.isPayAfterService] =
          isPayAfterService ? '1' : '0';
    }
    if (salonAbout != null) {
  ...
  Future<RestResponse> addStaffToSalon({
    ...
    var request = http.MultipartRequest(
      ConstRes.aPost,
      Uri.parse(ConstRes.addStaffToSalon),
    );
  ...
  Future<RestResponse> changeStaffStatus({
    required int staffId,
    required int status,
  }) async {
    final response = await http.post(
      Uri.parse(ConstRes.changeStaffStatus),
      headers: {ConstRes.apiKey: ConstRes.apiKeyValue},
      body: {
        ConstRes.salonId_: ConstRes.salonId.toString(),
        ConstRes.staffId: staffId.toString(),
        ConstRes.status: status.toString()
      },
    );
    final responseJson = jsonDecode(response.body);
    return RestResponse.fromJson(responseJson);
  ...
  Future<RestResponse> deleteStaff({
    required int staffId,
  }) async {
    final response = await http.post(
      Uri.parse(ConstRes.deleteStaff),
      headers: {ConstRes.apiKey: ConstRes.apiKeyValue},
      body: {
        ConstRes.salonId_: ConstRes.salonId.toString(),
        ConstRes.staffId: staffId.toString()
  ...
  Future<Staff> fetchAllStaffOfSalon() async {
    var request = http.MultipartRequest(
      ConstRes.aPost,
